        ...
        void)
{
    ...
    /* Producer endpoint 1 configuration */
    ...
    /* Producer endpoint 2 configuration */
...
///////////////////////////////// 2
	...
	dmaCfg2.prodSckId = CY_FX_PRODUCER_USB_SOCKET_2; //CY_U3P_UIB_SOCKET_PROD_2
	dmaCfg2.consSckId = CY_U3P_CPU_SOCKET_CONS;
	...
	apiRetStatus = CyU3PDmaChannelCreate (&glChHandleBulkLpIn,
			CY_U3P_DMA_TYPE_MANUAL_IN, &dmaCfg2);
	if (apiRetStatus != CY_U3P_SUCCESS)
	{
		CyU3PDebugPrint (4, "CyU3PDmaChannelCreate failed, Error code = %d\n", apiRetStatus);
		CyFxAppErrorHandler(apiRetStatus);
    ...
    dmaCfg2.consSckId = CY_FX_CONSUMER_USB_SOCKET_2;
...
//    }
	...
	CyU3PDebugPrint (4, "%s() (%d) : \n", __func__, __LINE__);

	for (;;)
	{
		//CyU3PDebugPrint (4, "%s() (%d) : \n", __func__, __LINE__);
		CyU3PDebugPrint (4, "%s() (%d) : glIsApplnActive=%d\n", __func__, __LINE__, (int)glIsApplnActive);
		if (glIsApplnActive)
		{
			...
			CyU3PDebugPrint (4, "%s() (%d) : status=%x\n", __func__, __LINE__, status);
			//CyU3PMemCopy (outBuf_p.buffer, inBuf_p.buffer, inBuf_p.count);
