...
CyU3PDmaChannel glChHandleSlFifoPtoU[4];   	/* DMA Channel handle for P2U transfer. */
...
void
CyFxUartLplnInit (void) //CyFxUartLpApplnInit (void)
{
...
////////////////// EndPoint SETTING  ////////////////////////////////////////////
	...
	epCfg.burstLen = BURST_LEN; // epCfg.burstLen = 1; //test 1

	for (i=0;i<4;i++)
	{
		 /* Consumer endpoint (i+1) configuration */
...
CyFxSlFifoApplnInit (void)
{
   ...
   CyU3PGpifSocketConfigure (3,CY_U3P_PIB_SOCKET_3,6,CyFalse,1);   //test 1
	...
	apiRetStatus = CyU3PGpifSMStart (RESET, RESET);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyFxAppErrorHandler(apiRetStatus);
    }
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        if (actualCount != 0)
        {
            ...
            glEP1OUTBuffer.cmd = rxTxByte;
            glEP1OUTBuffer.data[0] = actualCount;
            glEP1OUTBuffer.size = 1;
            SendHost();
            /* Send the byte */
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&slFifoAppThread,           /* Slave FIFO app thread structure */
                          ...
                          1,//CYU3P_NO_TIME_SLICE,                     /* No time slice for the application thread */
    ...
    ptr2 = CyU3PMemAlloc (CY_FX_UARTLP_THREAD_STACK);

    /* Create the thread for the application */
    retThrdCreate = CyU3PThreadCreate (&UartLpAppThread,           /* UART Example App Thread structure */
                          "20:UART_loopback_register_mode",        /* Thread ID and Thread name */
