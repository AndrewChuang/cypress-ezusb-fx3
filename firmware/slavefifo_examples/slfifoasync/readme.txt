...
ASYNCHRONOUS SLAVE FIFO PROTOCOL EXAMPLE
----------------------------------------
  ...
  A master device that implements the Cypress defined Async Slave FIFO
  protocol is required to perform data transfers with this application.

  This application example does the following:

  1. Configures the GPIF II interface to implement the Async Slave FIFO
     protocol.
  ...
  Files:
    ...
    * cyfxslfifoasync.h    : C header file that defines constants used by
    ...
    * cyfxslfifoasync.c    : Main C source file that implements this example.
