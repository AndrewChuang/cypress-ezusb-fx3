...
USB BULKLOOP-AUTO_ONE_TO_MANY EXAMPLE
-------------------------
  ...
  endpoints (1-OUT, 1-IN and 2-IN).  The application loops back any data that 
  it receives on the bulk OUT endpoint to the two bulk IN endpoints in an 
  interleaved fashion.

  The loopback is achieved with the help of a multichannel DMA AUTO_ONE_TO_MANY
  ...
  Files:
    ...
    * cyfxbulklpautoonetomany.h : Constant definitions for the bulk loop application.
    ...
    * cyfxbulklpautoonetomany.c : Main C source file that implements the bulk 
      loopback example.
