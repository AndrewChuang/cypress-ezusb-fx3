...
USB BULKLOOP-MANUAL_MANY_TO_ONE EXAMPLE
-------------------------
  ...
  endpoints (1-OUT, 2-OUT and 1-IN).  The application loops back any data 
  that it receives on the two bulk OUT endpoints to the bulk IN endpoint in 
  an interleaved fashion.

  The loopback is achieved with the help of a multichannel DMA 
  MANUAL_MANY_TO_ONE which is created connecting the three endpoints. This
  application receives data on the 1-OUT and 2-OUT endpoints from the USB host
  in an interleaved fashion. CPU is signalled of the incoming data via a DMA
  callback. CPU then sends the same data back to the host through the 1-IN
  endpoint.
  ...
  Files:
    ...
    * cyfxbulklpmanmanytoone.h : Constant definitions for the bulk loop 
    ...
    * cyfxbulklpmanmanytoone.c : Main C source file that implements the bulk 
      loopback example.
