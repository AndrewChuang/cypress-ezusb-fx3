                     ...
                     CyBool_t isRead)
{
    ...
    {
        temp = CyU3PI2cErrorRecovery (CyFalse);
    }
    if (temp != CY_U3P_SUCCESS)
    {
        CyU3PI2cReleaseLock();
...
CyU3PI2cTransmitBytes (
                       ...
                       uint32_t retryCount)
{
    uint32_t index, timeout, flag, status, temp;
    ...
    if (glIsI2cDma)
    {
        /* Since this is just a read, lock is not acquired. */
        return CY_U3P_ERROR_NOT_CONFIGURED;
    }
                      ...
                      uint32_t retryCount)
{
    ...
    /* Since this variable is just read, the lock
    * is not acquired at this point. */
    if (glIsI2cDma)
    {
        return CY_U3P_ERROR_NOT_CONFIGURED;
    }
    ...
    flag = CY_U3P_LPP_I2C_ERROR | CY_U3P_LPP_I2C_TIMEOUT |
                    ...
                    uint32_t retryCount)
{
    ...
    do 
    {
        ...
        status = CyU3PI2cSendCommand (preamble, 0, CyFalse);
        if (status != CY_U3P_SUCCESS)
        {
            break;
        }
