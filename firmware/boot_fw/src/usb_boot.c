        ...
        case CY_U3P_BOS_DESCR:
            {
                ...
                len = gpUsbDescPtr->usbSSBOSDesc_p[2];
                break; 
            }
        ...
        )
{
    ...
    if (CyFx3BootUsbSetClrFeature (0, &gEP0) != 0)
    {
        return eStall;
    }

    if ((gEP0.bmReqType == USB_REQ_ENDPOINT) || (gEP0.bmReqType == USB_REQ_INTERFACE))
    {
        return eStatus;
    }
 
    if ((gEP0.bmReqType == USB_REQ_DEV) && (CyFx3BootUsbGetSpeed () == CY_FX3_BOOT_SUPER_SPEED))
    {
        if (gEP0.bVal0 == 48)
        {
        ...
        ) 
{
    ...
    if (CyFx3BootUsbSetClrFeature (1, &gEP0) != 0)
    {
        return eStall;
    }
...
myUsbBoot ()
{
    ...
    /* Run USB circuitry off of internal power, not host power. */
    CyFx3BootUsbVBattEnable (CyTrue);
