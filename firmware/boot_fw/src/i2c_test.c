...
## ===========================
*/
/*This file has the implementation of fw2lpp firmware test bench
*/
        ...
        void)
{
    ...
    uint8_t writeBuffer[I2C_DMA_XFER_SIZE];
    uint8_t readBuffer[I2C_DMA_XFER_SIZE];
    uint32_t byte_count;
    uint16_t i = 0;
    uint16_t j = 0;
    ...
    for (j = 0; j < 250; j++)
    {
        ...
        status = CyFx3BootI2cDmaXferData (CyFalse, (uint32_t)writeBuffer, I2C_DMA_XFER_SIZE, 200);
        if (status != CY_FX3_BOOT_SUCCESS)
        {
            return status;
        }
        ...
        status = CyFx3BootI2cDmaXferData (CyTrue, (uint32_t)readBuffer, I2C_DMA_XFER_SIZE, 200);
        if (status != CY_FX3_BOOT_SUCCESS)
        {
            return status;
        }
