        ...
        void )
{
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        status = CyFx3BootUartDmaXferData (CyTrue, (uint32_t)buffer, bytesToRecv, CY_FX3_BOOT_WAIT_FOREVER);
        if (status != CY_FX3_BOOT_SUCCESS)
        {
            return status;
        }
        ...
        status = CyFx3BootUartDmaXferData (CyFalse, (uint32_t)buffer, bytesToRecv, CY_FX3_BOOT_WAIT_FOREVER);
        if (status != CY_FX3_BOOT_SUCCESS)
        {
            return status;
        }
