...
By default the application doesn't enable the no re-enumeration feature. The noReEnum parameter 
of the function CyFx3BootUsbStart () is to be set to CyTrue if the no re-enumeration feature is
to be made use of. Corresponding changes to the final application are also required. The final 
application is just expected not to set the descriptors and issue the connect call if the no 
re-enumeration feature is to be made use of. When this feature is used the descriptors across 
the booter and the final application are expected to be same.
