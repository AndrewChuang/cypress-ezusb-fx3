...
void CBulkLoopDlg::OnStartBtn() 
{
    ...
    else
    {
        ...
        if (n != 0)
        {
            if((m_OutEndptComBox.GetCurSel()!=-1)&&(m_InEndptComBox.GetCurSel()!= -1))
            {				
            ...
            {
                MessageBox("Invalid Endpoint, Please select the endpoint number","BulkLoop",NULL);
                return;
            }			
        }
    }
...
void CBulkLoopDlg::OnResetBtn() 
{
    ...
    m_FillPatternComBox.ResetContent();
...
void CBulkLoopDlg::OnRefreshBtn() 
{
    ...
    m_InEndptComBox.ResetContent();  
    m_OutEndptComBox.ResetContent();
    m_SuccessCount.SetWindowText("0");
    m_FailureCount.SetWindowText("0");
...
BOOL CBulkLoopDlg::OnDeviceChange(UINT EventType, DWORD_PTR dwData)
{
    ...
    case DBT_DEVICEARRIVAL:
        ...
        {
            CString DevId = pdbch->dbcc_name+4;
            DevId.MakeUpper();
            size_t pos = 0;
            pos = DevId.Find("VID_04B4");
            if(pos)
            {
                bPnP_Arrival = true;
                bPnP_DevNodeChange = false;
